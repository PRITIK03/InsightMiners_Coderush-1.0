    ...
    <div className="bg-white rounded-2xl shadow-2xl h-full border border-gray-100 overflow-hidden">
      ...
      {favorites && favorites.length > 0 && (
        <div className="p-4 border-b border-gray-200 bg-yellow-50">
          <div className="font-semibold text-yellow-800 mb-2 flex items-center">
            <span className="mr-2">★</span> Favorite Locations
          </div>
          <div className="flex flex-wrap gap-2">
            {favorites.map((fav, idx) => (
              <button
                key={idx}
                className={`px-3 py-1 rounded-lg text-xs font-medium border transition-colors duration-200 ${fav.district === selectedDistrict && fav.area === selectedArea ? 'bg-yellow-300 text-yellow-900 border-yellow-400' : 'bg-white text-gray-700 border-gray-300 hover:bg-yellow-100'}`}
                onClick={() => {
                  setSelectedDistrict(fav.district);
                  setSelectedArea(fav.area);
                }}
                aria-label={`Switch to favorite: ${fav.district}${fav.area ? ', ' + fav.area : ''}`}
                title={`Switch to favorite: ${fav.district}${fav.area ? ', ' + fav.area : ''}`}
              >
                {fav.district}{fav.area ? `, ${fav.area}` : ''}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="flex border-b border-gray-200 overflow-x-auto bg-white">
        {tabs.map((tab) => (
          ...
          >
            <span className="mr-2 text-base group-hover:scale-110 transition-transform duration-300">{tab.icon}</span>
            {tab.label}
            {tab.badge && (
              <span className="ml-2 px-2 py-0.5 bg-gradient-to-r from-purple-500 to-pink-500 text-white text-xs rounded-full animate-pulse">{tab.badge}</span>
            )}
            {activeTab === tab.id && (
        ...
        {loading ? (
          <div className="flex justify-center items-center h-64">
            <div className="relative">
              ...
              <div className="mt-4 text-center text-gray-600 font-medium animate-pulse">Loading insights...</div>
            </div>
          </div>
        ) : !pollutionData ? (
          <div className="text-center p-12 animate-fadeIn">
            ...
            <button onClick={() => setPollutionData(null)} className="bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white px-6 py-2 rounded-lg font-medium transition-all duration-300 transform hover:scale-105">Refresh Data</button>
          </div>
        ) : (
